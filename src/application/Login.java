...
public class Login implements Initializable {
	...
	public void userLogin(ActionEvent event) throws IOException {
		if(loginChoice.getValue() == null)
			...
			checkAdminLogin();
		else if(loginChoice.getValue().equals("Librarian")) 
			LibAdminLogin();
	}
	...
	//ETHAN ADDED THIS
	private void LibAdminLogin() throws IOException {
		Main m = new Main();
		DatabaseConnection connectNow = new DatabaseConnection();
		Connection connectDB = connectNow.getConnection();
		String verifyLoginQuery = "SELECT count(1) FROM Lib WHERE username = '" + username.getText() + "' AND password = '" + password.getText() + "'";
		...
		try {
			...
			while(queryResult.next()) {
				if(queryResult.getInt(1) == 1) {
					loginError.setText("Success");
					m.changeScene("LibDashBoard.fxml");
				}
				else {
					loginError.setText("Log-in Error! Try again");
				}
